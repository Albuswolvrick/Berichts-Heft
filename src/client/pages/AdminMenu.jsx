...
import '../../../public/css/AdminMenu.css';
        ...
        <li>
            {isEditing ? (
                <div className="edit-user-form">
                    ...
                    <button onClick={handleUpdate} className="btn btn-ready">{t('admin_menu.action_save')}</button>
                    <button onClick={() => setIsEditing(false)} className="btn">{t('admin_menu.action_cancel')}</button>
                </div>
            ) : (
                <div className="user-details">
                    <span>{user.name} ({user.email}) - {user.role}</span>
                    <div>
                        <button onClick={() => setIsEditing(true)} className="btn btn-primary">{t('admin_menu.action_edit')}</button>
                        <button onClick={() => setIsChangingPassword(!isChangingPassword)} className="btn btn-primary">{t('admin_menu.action_change_password')}</button>
                        <button onClick={() => onDelete(user.id)} className="btn btn-logout">{t('admin_menu.action_delete')}</button>
                    </div>
                </div>
            )}
            {isChangingPassword && (
                <div className="edit-user-form">
                    ...
                    <button onClick={handlePasswordChange} className="btn btn-ready">{t('admin_menu.action_save_password')}</button>
                    <button onClick={() => setIsChangingPassword(false)} className="btn">{t('admin_menu.action_cancel')}</button>
                </div>
            )}
        </li>
    );
    ...
    return (
        <div className="admin-menu">
            ...
            <div className="user-management">
                <h3>{t('admin_menu.user_management')}</h3>

                <form onSubmit={handleCreateUser} className="create-user-form">
                    <h4>{t('admin_menu.create_user')}</h4>
                    ...
                    <button type="submit" className={`btn btn-primary ${creationStatus === 'error' ? 'flash-error' : ''}`}>
                        {t('admin_menu.create_user')}
                    </button>
                </form>

                <h4>{t('admin_menu.existing_users')}</h4>
                {users.length === 0 ? (
                    <p>{t('admin_menu.no_users')}</p>
                ) : (
